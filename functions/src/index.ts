...
const CURRENT_PROJECT_ID = process.env.GCLOUD_PROJECT ?? process.env.GOOGLE_CLOUD_PROJECT ?? "";
...
const PROMOTION_SOURCE_URL =
    `https://${PROMOTION_REGION}-mrt2-app-dev.cloudfunctions.net/getPromotionContent`;
...
export function computeContiguousLastGeneratedDate(
    startDate: string,
    numDays: number,
...
async function logOpsAlert(source: string, message: string, detail: string): Promise<void> {
    try {
...
async function generateForModality(
    ...
    for (let batchStart = 0; batchStart < numDays; batchStart += BATCH_SIZE) {
        ...
        try {
            ...
            let batchCount = 0;
            const batchWrittenDates: string[] = [];
            ...
            if (batchCount > 0) {
                ...
                for (const d of batchWrittenDates) writtenDates.add(d);
            }
        } catch (err) {
    ...
    const bufferStatusUpdate: Record<string, unknown> = {
    ...
    if (errors > 0) {
        await logOpsAlert(
            "generateForModality",
            `${modality} buffer generation had ${errors} error(s) — requested ${startDate} + ${numDays}d, ` +
            `contiguous coverage reached ${contiguousThroughDate ?? "(none — startDate itself failed)"}.`,
            `written=${written} errors=${errors} startDate=${startDate} numDays=${numDays}`,
        );
    }

    logger.info(
        `PROJ-42: ${modality} — ${written} readings written, ${errors} errors. ` +
        `Contiguous buffer through ${contiguousThroughDate ?? "(unchanged — no new dates confirmed)"}.`
    );
...
function serializeForPromotion(data: DocumentData): Record<string, unknown> {
    const out: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(data)) {
        out[k] = v instanceof Timestamp
...
function reviveTimestamps(data: Record<string, unknown>): Record<string, unknown> {
    const out: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(data)) {
        const tagged = v as { __timestamp?: boolean; seconds?: number; nanoseconds?: number } | null;
        out[k] = tagged && typeof tagged === "object" && tagged.__timestamp === true
            ? new Timestamp(tagged.seconds ?? 0, tagged.nanoseconds ?? 0)
            : v;
    }
    return out;
}
...
async function fetchPromotionContent(
    collectionName: "daily_readings" | "crossword_puzzles",
    fromDate: string,
    toDate: string,
): Promise<PromotionResponse["docs"]> {
    const auth = new GoogleAuth();
    const client = await auth.getIdTokenClient(PROMOTION_SOURCE_URL);
    const response = await client.request<PromotionResponse>({
...
async function syncCollectionFromDev(
    collectionName: "daily_readings" | "crossword_puzzles",
    fromDate: string,
    toDate: string,
...
export const getPromotionContent = onRequest({
    ...
    if (
        (collectionName !== "daily_readings" && collectionName !== "crossword_puzzles") ||
