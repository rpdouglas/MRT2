...
* **PROJ-74 — Legacy Vault Unlock Hang on Decrypt-Mismatch.** Full spec exists at `docs/projects/74_LEGACY_VAULT_UNLOCK_HANG.md`. Found while writing `PROJ-73`'s `EncryptionContext.test.tsx` coverage: a decrypt mismatch during legacy (pre-`pinVerifier`) vault unlock makes `performUnlock` resolve `true` without ever setting `isVaultUnlocked`, which hangs `VaultGate.tsx`'s unlock button indefinitely with no error. Narrow blast radius (pre-PROJ-65 accounts only), but a real hang for whoever hits it. Status: ⚪ Planned — has an open design question (§2) to confirm before implementation.
