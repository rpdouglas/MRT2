...
| **[`walt-journal-history.webp`](file:///workspaces/MRT2/docs-site/public/screenshots/walt-journal-history.webp)** | `/journal?mockUser=walt&tab=history` | Walt | Journal History. Displays a feed of past entries grouped by Year and Month, indicating mood icons and encryption badges (🔒). See §6 for the 2026-09-06 fix note. |
...
| **[`walt-journal-insights.webp`](file:///workspaces/MRT2/docs-site/public/screenshots/walt-journal-insights.webp)** | `/journal?mockUser=walt&tab=insights` | Walt | Journal Insights. Displays weekly and monthly mood trends, somatic balance charts, and check-in word cloud frequencies. See §17 for the 2026-09-06 fix note. |
...
*   **Visual Structure**: A scrollable list of entries grouped chronologically by Month/Year header blocks (e.g. "2026 — 10 Entries" with a "SEPTEMBER" sub-header). Each card shows the mood badge, weather badge, and a snippet of the entry content.
*   **Semantic Meaning**: This shows the ZK database decryption at the UI boundary. The historical data feed is decrypted on the fly using the user's PIN-derived vault key. **Fixed 2026-09-06** — this capture previously rendered "No entries found" instead of the populated feed described above. Root cause: mock journal timestamps use a duck-typed `Timestamp` wrapper (`createMockTimestamp()` in `mockData.ts`, not a real `firebase/firestore` `Timestamp` instance), and both `JournalHistory.tsx`'s current-year filter and the shared `groupItemsByYearAndMonth` utility (`src/lib/grouping.ts`) checked `instanceof Timestamp`, which is `false` for the mock wrapper — every entry was silently dropped. Both now duck-type on the presence of a `.toDate` method instead.
...
*   **Visual Structure**: Indigo/violet theme (`THEME.journal` — fixed for the Journal feature, independent of Walt's own "emerald" persona hero color used elsewhere like the Dashboard). Top stat tiles ("10 Entries", "7.9 Avg Mood", "Active" streak flame), then three stacked cards in one tall capture: a "Weekly Rhythm" bar chart (avg mood by day of week, prev-30-days vs last-30-days), an "Emotional Velocity" line chart (mood flow vs temperature over the last ~2 weeks), and a "Recurring Themes" word cloud.
*   **Semantic Meaning**: Interactive somatic tracking and vocabulary frequency metrics for long-term recovery insight. **Corrected/Fixed 2026-09-06** — this entry previously said "Emerald theme," which was never accurate (the Journal feature always uses its own fixed indigo/violet theme, not the persona's hero color); the capture itself also previously only showed the first chart and part of the second, cutting off the word cloud below the fold. Root cause of the capture bug: `Journal.tsx` renders its tabs inside an internally-scrolling `h-[100dvh]` shell rather than normal page scroll, so Playwright's `fullPage` screenshot option (which only follows document/body scroll) never reached the clipped content. `scripts/generate_screenshots.js` now measures the real content height of the tagged scroll container (`[data-screenshot-scroll-root]`) and grows the viewport to fit it before capturing this target, producing the single tall image described above.
