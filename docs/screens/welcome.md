...
- **Android TWA branch (PROJ-116 §9, 2026-09-09):** before any of the marketing JSX below is even constructed, `isAndroidTWA()` (`document.referrer.startsWith('android-app://')`, same check PROJ-68/105 use for billing) branches to `WelcomeAndroidApp` instead — logo, one tagline, the trust bar (crisis link included, unconditionally), and the auth form only. No hero, feature strip, quiz, or showcase. `twa-manifest.json`'s `startUrl` is `"/"`, so this is a Play Store visitor's actual first screen on a logged-out cold launch; they already made the highest-intent decision (install) before reaching it. Everything below this point describes the web/iOS-PWA experience only.
