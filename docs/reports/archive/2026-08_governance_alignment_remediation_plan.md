...
| ~~320px (iPhone SE) viewport passes~~ **Done 2026-08-03** | PROJ-90, 96, 98 | Checked via a temporary Playwright script (320x568 viewport, real dev server + emulators, screenshots reviewed by eye) rather than a real device — Welcome and Login (medical disclaimer wraps cleanly, no clipping), Dashboard and Tasks (no overflow; both have components that truncate gracefully by design at this exact width, per PROJ-41/PROJ-78), and VaultGate's PIN-setup screen. All passed. Didn't reach a live PROJ-98 `aria-label` component specifically (the test user has no vault PIN yet), but `aria-label` is a non-visual attribute with no plausible layout impact, and the existing axe-core CI gate (`e2e/golden-paths/a11y.spec.ts`) already covers accessibility regressions on every push — this is close enough to call the item done, not a gap worth chasing further. |
