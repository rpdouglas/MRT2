...
* [ ] **Security review recommended before production rollout** given this touches every user's key derivation, and given how close the rate-limiter no-op bug above came to shipping — the `zk-audit` + manual passes above substantially reduce risk but are not a substitute for external review.
