...
* [x] **Share still failing after the 2026-09-05 fix, root cause found and fixed 2026-09-06.** The `res.ok` check/error-surfacing fix above made the failure *visible* but didn't fix it — Share kept failing every time, now with the "Couldn't share the image" message instead of silence. Actual root cause: the Storage bucket has never had a CORS policy configured. `handleShare`'s `fetch(dailyImage.downloadUrl)` is a genuine cross-origin request to `firebasestorage.googleapis.com`; a browser blocks reading its response body without a matching CORS policy on the bucket, even though the identical URL loads fine in the modal's own `<img src>` (an `<img>` load isn't subject to the same restriction). This is a third instance of the same class of gap as the bucket-provisioning entry above: Storage's one-time infrastructure setup (enable API, create bucket, *and* configure CORS) isn't covered by `firebase deploy` or any emulator-backed rules test — CORS has no `firebase.json` field at all, it's GCS-bucket-level only. Fixed by adding a version-controlled `storage.cors.json` (GET-only, `origin: ["*"]` — deliberate: this bucket only ever serves non-sensitive editorial content per `CLAUDE.md`'s ZK boundary table, and the download-token URL already bypasses Security Rules regardless of origin, so a permissive CORS origin adds no new exposure) and applying it via `gcloud storage buckets update gs://<bucket> --cors-file=storage.cors.json` to `mrt2-app-dev` and `mrt2-app-prod`. **Not yet confirmed for `mrt2-app-uat`** — unlike the bucket-provisioning fix, this note doesn't establish whether a UAT bucket exists at all; check before assuming Daily Image works there. No code changes — `DailyImageModal.test.tsx`'s mocked-fetch tests can't exercise CORS at all (real network/browser behavior), so they weren't a false-negative here, just an inherently blind spot for this class of bug.
...
* [x] **Relocated that entry point out of `VibrantHeader` into the Daily Reading dropdown — 2026-09-14.** Reported directly: the header's right-side icon cluster (Help, SOS, and this button) no longer fit within the two `flex-1` boxes' natural equal-width split at typical viewport widths, so the center title visibly drifted left of center. Rather than widen the header, moved the trigger to `DynamicAnchorWidget.tsx`'s existing "Daily Reading" dropdown (the one listing `FELLOWSHIPS` — AA/NA) as a third `MenuItem`, "Today's Image" — a more natural home (a menu of "things to read/view today") than a bare header icon. `Dashboard.tsx` still owns `useDailyImage()`, `showDailyImageModal` state, `<DailyImageModal>`'s render, and the once-per-day auto-popup effect unchanged; it now passes `dailyImageAvailable`/`onViewDailyImage` down to `DynamicAnchorWidget` instead of `extraAction` down to `VibrantHeader`. `VibrantHeader.tsx` itself needed no change — `extraAction` was already optional and Dashboard was its only caller; the prop stays available for a future use. Also fixed a latent test-environment gap this surfaced: no prior test had ever actually opened this HeadlessUI `Menu` (only its non-dropdown click path), so `jsdom`'s missing `ResizeObserver` (which `Menu` needs for anchor positioning once opened) had never thrown — added a minimal `ResizeObserver` stub to `src/test/setup.ts`. New tests in `DynamicAnchorWidget.test.tsx` (2) and `Dashboard.test.tsx` (2). Full suite (824 tests) and lint clean.
