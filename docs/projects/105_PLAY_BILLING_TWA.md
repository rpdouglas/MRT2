...
**Status:** 🟡 **Resumed 2026-09-10 — AAB published, subscription-creation blocker cleared; `premium.monthly` product not yet created.** Code complete 2026-09-02 (deployed to prod same day). Sibling project `PROJ-07` (Play Store submission) is now **Live in Production**, not paused — see that spec.

**2026-09-10 — v1.9.13/`appVersionCode` 8 reached Published.** Play Console now shows the ability to create a subscription product (the "Upload a new APK" prompt from 2026-09-09 is gone), confirming Google detected the `com.android.vending.BILLING` permission in the live release. This clears blocker §6 item 3. **Not yet done:** the `premium.monthly` product itself hasn't been created in Play Console — the values are already decided below, ready to enter.
...
3. ⬜ **UNBLOCKED 2026-09-10 (AAB v1.9.13/`appVersionCode` 8 reached Published) — Play Console: create the subscription product.** Play Console → Monetize → Products → Subscriptions → create `premium.monthly` priced at $3.99/mo — must match `VITE_PLAY_BILLING_PRODUCT_ID`. The "Upload a new APK" prompt is gone, confirming Play now detects the `com.android.vending.BILLING` permission in the published release. Product itself not yet created — use the values already decided above (§ "Subscription product values decided").
...
5. ⬜ **Play Console: point RTDN at the topic** (no longer blocked — do this once the subscription product exists). Monetize → Monetization setup → Real-time developer notifications → `projects/mrt2-app-prod/topics/play-billing-rtdn`.
