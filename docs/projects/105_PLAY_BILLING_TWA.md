...
**Status:** ✅ **Shipped & Live in Production (verified 2026-09-15).** Release v1.9.13 reached Published in Play Console. Real-device purchase testing completed and verified working end-to-end on Android device (token persisted, verified via Cloud Functions, premium tier granted). Defensively trimmed productId whitespace in PR #236 to ensure clean SKU resolution.

**2026-09-15 — Real-device purchase verified working.** Native Google Play Billing purchase bottom sheet opens on device, test purchase completes with "Test card, always approves", purchaseToken verified, and Supporter tier unlocks immediately. Defensively trimmed `productId` in `PremiumUpgrade.tsx` and `playBilling.ts` to prevent whitespace SKU mismatches (PR #236). All §6 checklist items closed.

**2026-09-10 — `premium.monthly` subscription product created and Active.** Entered with the values already decided below (Product ID `premium.monthly`, base plan `monthly`, $3.99 USD/mo, auto-renewing, all regions, no trial). This clears blocker §6 item 3 for real (superseding the "not yet done" note below, which was true earlier the same day). **Next:** §6 item 5 (point RTDN at the existing Pub/Sub topic — no longer blocked now that the product exists) and item 7 (real-device purchase test, ideally via a Play Console license-test account before taking real money).

**2026-09-10 — v1.9.13/`appVersionCode` 8 was uploaded and detected by Play Console.** ~~Play Console now shows the ability to create a subscription product (the "Upload a new APK" prompt from 2026-09-09 is gone), confirming Google detected the `com.android.vending.BILLING` permission in the live release. This clears blocker §6 item 3.~~ **Corrected 2026-09-12: this was misread as "Published" — the release was actually still "In review" (see status line above). What's true is narrower: Play detected the permission in the *uploaded* build, which was enough to unlock subscription-product creation, but the release itself had not gone live.**

**2026-09-09 — Play Console payment-method verification cleared**, unblocking subscription-product creation for the first time. Attempting to create `premium.monthly` immediately surfaced a **second, previously-undiscovered blocker**: the Subscriptions page showed "Your app doesn't have any subscriptions yet" with an "Upload a new APK" prompt instead of a "Create subscription" button. Root-caused (not guessed — verified directly against the actual TWA scaffold and a real build): `~/mrt-android/twa-manifest.json` had never enabled Bubblewrap's `playBilling` feature (`"features": { "playBilling": { "enabled": true } }`) or `alphaDependencies`, so every AAB shipped for this app to date — including the one live in Production — never declared the `com.android.vending.BILLING` permission Google's own docs say is required before an app can offer any in-app product. Google Play Console gates subscription/product creation behind detecting that permission in an uploaded build.

**Fixed the same day:** `twa-manifest.json` updated (`playBilling`/`alphaDependencies` enabled, version bumped to `1.9.13`/`appVersionCode` 8); `bubblewrap update` regenerated the Android project; the 3 Phase 3.5 hardening fixes that `bubblewrap update` always reverts (`androidbrowserhelper` back to `2.7.3`, `proguardFiles`, `app/proguard-rules.pro`) were reapplied and verified via a clean `./gradlew clean bundleRelease assembleRelease` (confirmed `com.android.vending.BILLING` + `InAppBillingService.BIND` now present in the merged manifest, R8 `mapping.txt` present). `bubblewrap build` run (keystore password, account-holder only) to produce the real signed AAB. New release uploaded to **Production** (same track as the existing live release — not a new testing track) and is currently **in review**. The `VITE_PLAY_BILLING_PRODUCT_ID` GitHub Secret (`premium.monthly`) is now set, and the deploy-workflow wiring gap that would have silently no-op'd it (§6 item 6 below) was found and fixed the same day.

**To resume once the review clears:** confirm the release reached **Published** (not just "in review" — Play Console only unlocks subscription creation once the release is actually live, not merely uploaded), then create the `premium.monthly` subscription product per the values already decided below, and continue §6's checklist.

**Subscription product values decided (ready to enter once unblocked):** Product ID `premium.monthly`, Name "MRT Supporter — Monthly", base plan ID `monthly`, Monthly billing period, Auto-renewing, $3.99 USD default price (auto-converts to other currencies), all available countries/regions, no trial/offer at launch. Keep this subscription to exactly one base plan — a 2023-10-14 Bubblewrap GitHub issue (`GoogleChromeLabs/bubblewrap#830`, still open/unresolved, community-reported not Google-confirmed) indicates the Digital Goods API/Payment Request API bridge used by this TWA can only query/purchase by the subscription-level Product ID, not an individual base plan ID — our client code already only ever passes the bare Product ID, matching the pattern reported to work.
...
**NOT built — external, one-time setup only the account owner can do (no code, can't be automated from here). `PROJ-07` reached Production 2026-09-09, clearing the original blocker on this section — see the 2026-09-09 finding above for the second, unrelated blocker that surfaced next:**
...
3. ✅ **Play Console: subscription product created and Active** (`premium.monthly`, $3.99/mo, 2026-09-10) — matches `VITE_PLAY_BILLING_PRODUCT_ID`, entered with the values already decided above (§ "Subscription product values decided").
...
5. ✅ **Play Console: RTDN pointed at the topic and verified end-to-end (2026-09-12).** `google-play-developer-notifications@system.gserviceaccount.com` granted `roles/pubsub.publisher` on `play-billing-rtdn` (was missing — required, and Play Console gives no error if it's absent, notifications just never arrive). Play Console → Monetize → Monetization setup → Real-time developer notifications wired to `projects/mrt2-app-prod/topics/play-billing-rtdn`. Play's "Send test notification" confirmed delivered — `handlePlayRTDN` logged `"non-subscription notification, ignoring"` at 2026-09-12 00:08:14 UTC, the correct no-op path (`functions/src/index.ts:1512`) for Play's generic test payload, which isn't a real `SubscriptionNotification`.
6. ✅ **`VITE_PLAY_BILLING_PRODUCT_ID`** GitHub Secret set (`premium.monthly`), 2026-09-09. **Found and fixed the same day:** the value was never actually wired into `.github/workflows/deploy.yml`'s secrets → env → build-env chain like every other `VITE_*` variable — it would have silently no-op'd (fails safe: `PremiumUpgrade.tsx` shows "Play Billing is not configured for this build." rather than crashing) even with the secret set. Fixed in PR #223 (merged), mirroring the existing `VITE_STRIPE_PREMIUM_PRICE_ID` single-shared-secret pattern.
7. ✅ **Real-device testing** — verified working end-to-end on Android device (2026-09-15). Native Google Play Billing purchase bottom sheet opens and completes, token verified via `verifyPlayPurchase`, and Supporter tier activated. Defensively trimmed SKU whitespace in PR #236.
8. ✅ **`docs/legal/PLAY_STORE_DATA_SAFETY_DRAFT.md`** — reviewed 2026-09-09: already correctly generalized regardless of payment processor (*"Payment info (card details): No"*, *"Purchase history / subscription tier... Yes (tier only)"*) — no change needed. Confirming the *live* Play Console form still matches is tracked under `PROJ-07`, not here (already closed per that spec's 2026-09-09 update).
