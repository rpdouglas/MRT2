...
**Status:** 🟡 **RTDN verified end-to-end (2026-09-12); only real-device purchase testing remains.** Code complete 2026-09-02 (deployed to prod same day). Sibling project `PROJ-07` (Play Store submission) is now **Live in Production**, not paused — see that spec. `premium.monthly` is created and Active, and Play's RTDN test notification confirmed reaching `handlePlayRTDN` — see §6 item 5. Last open item is §6 item 7 (real-device purchase test, ideally via a Play Console license-test account).
...
**2026-09-10 — v1.9.13/`appVersionCode` 8 reached Published.** Play Console now shows the ability to create a subscription product (the "Upload a new APK" prompt from 2026-09-09 is gone), confirming Google detected the `com.android.vending.BILLING` permission in the live release. This clears blocker §6 item 3. **Not yet done (as of earlier this session):** the `premium.monthly` product itself hasn't been created in Play Console — the values are already decided below, ready to enter.
