...
**Status:** 🟡 **Resumed 2026-09-09 — awaiting Play review before subscription creation.** Code complete 2026-09-02 (deployed to prod same day). Sibling project `PROJ-07` (Play Store submission) is now **Live in Production**, not paused — see that spec.
...
3. ⬜ **BLOCKED on the new AAB (v1.9.13/`appVersionCode` 8) reaching Published, not just uploaded — Play Console: create the subscription product.** Play Console → Monetize → Products → Subscriptions → create `premium.monthly` priced at $3.99/mo — must match `VITE_PLAY_BILLING_PRODUCT_ID`. Payment-method verification cleared 2026-09-09; the remaining blocker is Play detecting the `com.android.vending.BILLING` permission in a **published** release (see the 2026-09-09 finding above — the button showed "Upload a new APK" even with the app already live, because no prior release had ever declared this permission).
