...
**Status:** 🟡 Code complete 2026-09-02 (deployed to prod same day) — blocked on Play Console payment-method verification (account-side, in progress, no ETA), which is gating subscription-product creation and therefore real end-to-end testing. Pub/Sub topic (`play-billing-rtdn`) already created; API access to the Cloud Functions runtime service account confirmed 2026-09-03 (see §6 item 4's correction — the account originally documented there was never real). **2026-09-03: added a dev/emulator-only mock purchase path** (below) so the full flow can be exercised while blocked, without waiting on the real product to exist.
