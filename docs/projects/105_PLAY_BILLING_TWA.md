...
**Status:** 🟡 **`premium.monthly` created and Active in Play Console (2026-09-10).** Code complete 2026-09-02 (deployed to prod same day). Sibling project `PROJ-07` (Play Store submission) is now **Live in Production**, not paused — see that spec. Remaining work is external Play Console setup (RTDN topic wiring) plus real-device purchase verification — see §6 items 5 and 7.
...
5. ⬜ **NEXT — Play Console: point RTDN at the topic** (unblocked now that the subscription product exists). Monetize → Monetization setup → Real-time developer notifications → `projects/mrt2-app-prod/topics/play-billing-rtdn`. Without this, `handlePlayRTDN` never fires on renewal/cancellation/refund — `tier` stays correct only until the next event Google would have pushed.
