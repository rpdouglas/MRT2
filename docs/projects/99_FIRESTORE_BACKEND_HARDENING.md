...
### Phase 3: Index reconciliation — ✅ Shipped (partially — see note)
...
* **Not done, and not achievable from this environment:** the plan's "pull the actual deployed composite indexes for `mrt2-app-prod` and diff" step. This sandbox has no Firebase CLI authentication and no access to production credentials — the two indexes above were derived entirely from static analysis of the query code, not verified against what's actually live. Validated the JSON is syntactically well-formed by starting the Firestore emulator with it (loads without error), which is the most this environment can confirm. **Residual manual step for whoever has prod access:** run `firebase deploy --only firestore:indexes --project mrt2-app-prod` (or check the console) to confirm these two match reality and nothing else is silently relied upon.
