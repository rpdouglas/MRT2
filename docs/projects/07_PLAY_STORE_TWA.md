...
**Status:** ✅ **Live in Production (confirmed 2026-09-09).** The app is published and downloadable from Google Play. Sprint 9.1 implemented and code-verified 2026-07-19 (Epics 1-3; see §4/§5). Sprint 9.2 (Epic 4): TWA built, signed, uploaded, device-verified working (correct icon, no URL bar — a real `www`-subdomain-redirect Digital Asset Links bug was found and fixed along the way), and promoted through to Production. The account's Organization-type exemption from the "12 testers/14 days" closed-testing gate (Internal Testing → Production directly) held as expected. A second production release (v1.9.13/`appVersionCode` 8, 2026-09-09) shipped afterward to add Play Billing support — see `docs/projects/105_PLAY_BILLING_TWA.md`.
