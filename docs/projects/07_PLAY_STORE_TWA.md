...
**Status:** ✅ **Live in Production (confirmed 2026-09-09) on v1.9.12/`appVersionCode` 7.** The app is published and downloadable from Google Play. Sprint 9.1 implemented and code-verified 2026-07-19 (Epics 1-3; see §4/§5). Sprint 9.2 (Epic 4): TWA built, signed, uploaded, device-verified working (correct icon, no URL bar — a real `www`-subdomain-redirect Digital Asset Links bug was found and fixed along the way), and promoted through to Production. The account's Organization-type exemption from the "12 testers/14 days" closed-testing gate (Internal Testing → Production directly) held as expected. **Correction 2026-09-12:** a follow-up release (v1.9.13/`appVersionCode` 8, uploaded 2026-09-09) to add Play Billing support was previously logged here as "shipped"/"Published" — that was wrong. Play Console's Releases page shows it still **"In review"** as of 2026-09-12; it has not gone live. The app users install today is still v1.9.12, which has no Play Billing support. See `docs/projects/105_PLAY_BILLING_TWA.md` for the corrected status and what's blocked on this.

**Closed checklist (previously tracked here as open, confirmed complete 2026-09-09 — this doc had drifted from reality by not being updated when each was actually done):**
- ✅ App-content checklist (11/11), Health apps declaration, TWA build/upload/device verification, Store Listing (description/icon/feature graphic/screenshots/category).
- ✅ **Content rating questionnaire (IARC)** — complete (required by Play before Production publishing).
- ✅ **Data Safety** — all categories (Personal info, Financial info, Device/other IDs, Location) confirmed saved (required by Play before Production publishing).
- ✅ **The Subway Test** (offline/airplane-mode check) — performed.
- 🟡 **PROJ-105 (Play Billing)** — code shipped, real root blocker found and fixed 2026-09-09 (the TWA's `twa-manifest.json` never declared the `playBilling` feature, so the uploaded AAB never carried the `com.android.vending.BILLING` permission Play Console requires before unlocking subscription creation — see that spec for full detail). New AAB built and uploaded to Production 2026-09-09; awaiting Play review before the `premium.monthly` subscription product can be created.
