...
**Status:** 🟡 In Progress — Sprint 9.1 implemented and code-verified 2026-07-19 (Epics 1-3; see §4/§5). Sprint 9.2 (Epic 4) substantially complete as of 2026-09-03: TWA built, signed, uploaded to Internal Testing (release 6, v1.9.11), device-verified working (correct icon, no URL bar — a real `www`-subdomain-redirect Digital Asset Links bug was found and fixed along the way). App-content checklist, Health apps declaration, and most of the Store Listing / Data Safety questionnaire have been walked through live in Play Console; Content rating questionnaire and full Data Safety category confirmation remain open. **Key 2026 policy finding (2026-09-03):** this Play Console account is an Organization account, which is exempt from the "12 testers / 14 days" closed-testing gate Google requires of new personal accounts — the promotion path is Internal Testing → Production directly, not through a mandatory closed test. See `docs/PLAY_STORE_BUBBLEWRAP_GUIDE.md` Phase 9 for the full researched breakdown and remaining pre-production gates. Separately, Play Billing (PROJ-105) subscription-product creation is blocked on Play Console payment-method verification (account-side, in progress, no ETA).
