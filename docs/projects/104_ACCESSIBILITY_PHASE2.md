...
* **Follow-up correction (2026-09-14):** the AppShell button was unconditional, so it also rendered on every *unlocked* page whose `VibrantHeader` already has its own SOS button — a visible duplicate button, reported directly. Fixed without reopening the reachability gap: `LayoutContext` gained `headerSOSMounted`/`setHeaderSOSMounted`; `VibrantHeader` sets it `true` on mount and `false` on unmount; `AppShell`'s floating button now renders only `{!headerSOSMounted && ...}`. Locked vault (no `VibrantHeader` mounts) and `WorkbookSession.tsx` (the one `VaultGate`-wrapped route with no `VibrantHeader` at all) both keep the fallback button; every other unlocked, headered route now shows exactly one. Required updating 13 test files whose `LayoutContext` mocks lacked the new setter (`setHeaderSOSMounted is not a function` once `VibrantHeader`'s mount effect ran) — `src/pages/__tests__/{GamesHub,Profile,UrgeSurfer,PremiumUpgrade,ToolsHub,ToolHistory}.test.tsx`, `src/components/tools/__tests__/GuidedWorkflowEngine.test.tsx`, `src/components/smart_tools/__tests__/{ThoughtRecordTool,CBATool,DentsTool,MorningIntentTool,FiveQuestionsTool,ABCTool}.test.tsx`. 818/818 full suite passing, lint clean.
