...
## 15. As-Built Note: Buffer Gap Fix + Generate-Once/Promote Architecture (2026-09-09)

**The bug.** Users reported the Dashboard's "Daily Reading" button silently falling back to the external fellowship link instead of opening the in-app `ReadingModal`. Investigation (not a code regression — `DynamicAnchorWidget.tsx`'s fallback logic was unchanged since original ship) traced this to `daily_readings` documents genuinely missing from Firestore for specific dates, confirmed via a real signed-in user's own Firestore read against `mrt2-app-dev` returning 404, despite `checkBufferHealth`'s nightly logs reporting "all buffers healthy."

**Root cause — two compounding gaps in `functions/src/index.ts`, both now fixed:**
1. `generateForModality` wrote `buffer_status.lastGeneratedDate` to the *nominal end of the requested range* regardless of which batches actually succeeded. A single failed batch (Gemini JSON-parse error, non-array response) or even one invalid reading within an otherwise-good batch left a permanent, invisible hole — `checkBufferHealth` only ever reads the water mark, never verifies the dates behind it exist. **Fixed:** the water mark now only advances through dates confirmed contiguously written (`computeContiguousLastGeneratedDate`, unit tested) — a gap simply isn't counted, so the next run's `startDate` naturally retries it. Self-healing instead of silent.
2. `checkBufferHealth`, when a buffer had fully expired (water mark fell behind "today"), deliberately resumed generation from *tomorrow* rather than the water mark — permanently orphaning every date in between, including "today" itself on whatever night this triggered. This is the more likely explanation for multiple modalities showing the same missing date at once (they refill together in one run, so a shared-cause outage hits all of them the same night). **Fixed:** always resumes from `lastGeneratedDate + 1`, never skips ahead — a bad outage now costs a few extra catch-up nights instead of a permanent gap.
3. The spec (§6, Job 1) always called for "sends alert to admin if buffer falls below critical" — this was never actually built (only `logger.error`, visible in Cloud Logging only). **Fixed:** `logOpsAlert()` now writes into the existing `client_errors` collection (tagged `userAgent: "cloud-function"`) on any batch error, surfaced for free in the existing admin-only `ErrorLogViewer.tsx` — no new admin UI needed.

**Separately identified during the same investigation, and folded into this same fix: triple-generation across environments.** `.github/workflows/deploy.yml` deploys `functions/` independently to `mrt2-app-dev`, `mrt2-app-uat`, and `mrt2-app-prod` on every push, with no environment gating on `checkBufferHealth`/`generateDailyCrossword`. Each project therefore ran its own independent nightly Gemini call — 3x the generation cost, and dev/uat/prod showing different LLM-generated text for "today's AA reading" rather than the same content promoted through environments. This also meant a buffer gap in one environment said nothing about the others.

**Fixed via a generate-once-in-dev / pull-into-uat-prod architecture:**
- Only `mrt2-app-dev` ever calls Gemini for `daily_readings` and `crossword_puzzles` (`IS_DEV_PROJECT` branch in `checkBufferHealth` / `generateDailyCrossword`).
- A new function, `getPromotionContent`, is the read boundary uat/prod pull through. Firestore/IAM has no collection-level scoping — a raw cross-project Firestore IAM grant for uat/prod to read dev would expose *all* of dev's Firestore (including real QA/demo-persona account data), not just these two editorial collections. `getPromotionContent` is deliberately narrow instead: it only ever returns `daily_readings`/`crossword_puzzles` for a bounded date range, and (via the `invoker` option on the Gen2 HTTPS function — Cloud Run/Functions IAM *does* support per-service invoker scoping, unlike Firestore) is only callable by uat's and prod's own default compute service accounts, never public. The trust boundary is "can call an endpoint that only ever returns these 2 collections," not "can read dev's database."
- uat/prod's `checkBufferHealth`/`generateDailyCrossword` now call `syncCollectionFromDev()` (Google-signed service-to-service ID token, `google-auth-library`'s `getIdTokenClient`) instead of generating, and merge-upsert the result — idempotent, self-healing the same way dev's own generation is.
- `daily_images` (PROJ-113) deliberately excluded from this pass — it's pool *selection*, not Gemini generation, so the 3x-cost concern doesn't apply; each environment showing a different rotation pick is a lower-priority consistency nit, not a cost or drift risk. Candidate for the same pattern later if that changes.

**Deploy prerequisite (manual, one-time, cannot be verified without a real deploy):** confirm `onRequest`'s `invoker` option actually produces the expected Cloud Run IAM binding on `getPromotionContent` (`gcloud run services get-iam-policy getpromotioncontent --project mrt2-app-dev --region northamerica-northeast1`), and confirm the computed `PROMOTION_SOURCE_URL` (`https://northamerica-northeast1-mrt2-app-dev.cloudfunctions.net/getPromotionContent`) matches what actually deploys.

**Remediation (folded into this same fix, not a separate patch):** rather than a bespoke backfill script, the currently-missing dates are healed by re-running the now-fixed pipeline itself — rolling `buffer_status.lastGeneratedDate` back to before the earliest confirmed gap and letting the fixed nightly job regenerate forward, gaplessly. uat/prod backfill automatically once on the sync path, from dev's now-complete range.