...
### Phase 3: Edge Cases
* [x] `navigator.onLine` false — today's puzzle is fetched once via `useQuery` (`staleTime`/`gcTime` set), then playable from cache; consistent with other games' offline behavior. **Now exercised by an automated Subway Test (2026-09-03)** — see §5.
...
* [x] **The Subway Test — closed 2026-09-03:** added to `e2e/golden-paths/subway.spec.ts`. Unlike Fast Lane, this game has no genuine multi-session resume state (solved in one sitting), so the test proves what actually applies here instead: the puzzle renders from Firestore's local cache offline, is fully solvable offline, and the completion write reaches the server once back online. `crossword_puzzles/{today}` is seeded via the Firestore Admin SDK directly (`e2e/fixtures/seedCrossword.ts`, reusing `useDailyCrossword.ts`'s own `MOCK_PUZZLE` fixture content), bypassing both the admin-write-only rule and the real Gemini generator for a deterministic, network-free test. Run for real against a live local emulator (`firebase emulators:exec ... playwright test`), not just written and assumed to pass.
* [x] **The "Lost PIN" Test — closed 2026-09-03:** 2 new unit tests in `rotation.test.ts` using a `gameId: 'daily-crossword'` fixture, explicitly proving both `executePinRotation` and `executeCryptoShredding` cover this game. The underlying sweep logic was already collection-wide (never branching on `gameId`), so no code change was needed — this closes the coverage gap explicitly rather than leaving it implicit. The Subway Test's final assertion (`encryptedStats` is real ciphertext) doubles as a live version of the same check.
...
5. **Generate-once-in-dev / promote-to-uat-prod (2026-09-09) — see `docs/projects/42_DAILY_READINGS.md` §15.** `generateDailyCrossword` previously ran independently in all three Firebase projects (no environment gating in `deploy.yml`'s functions deploy) — 3x the Gemini cost and no guarantee dev/uat/prod showed the same puzzle for a given date. Fixed alongside PROJ-42's buffer-gap fix: only `mrt2-app-dev` generates; uat/prod pull through the new `getPromotionContent` function instead. This crossword function's own "check today + tomorrow" existence-check design (see its inline comment) was already gap-safe and needed no change — the water-mark bug that PROJ-42 fixed is specific to `daily_readings`' far-ahead buffered-batch design, which crossword never used.
