...
* **Per-section ROSC pillar colors (rose/orange/purple/green), all sections uniformly "light frosted glass" when expanded, all collapsed by default (2026-07-21):** Product owner screenshot comparison against the ROSC Matrix specifically: ROSC's `ROSCPillCapsules.tsx` gives each of its 4 pillars (Health/Home/Purpose/Community) its own hue — pink/rose, orange, purple, green — inside one dark hero card, and asked for the same "one color per section" idea applied to the Tools Hub's 4 phases, plus the expanded body reading as "the header's theme, much lighter," using ROSC's own glassy styling cues. Added `gradA`/`gradB` (hex gradient-border stops) and literal `pillBg`/`pillText`/`hoverText` Tailwind class strings to each `PHASE_META` entry in `toolsRegistry.ts`, mapped by connotation rather than arbitrarily: `right-now`→rose (echoes ROSC's "Health," warm urgency), `before`→orange (echoes "Home," vigilance/preparedness), `after`→violet (echoes "Purpose," introspection — and already matched 3 of its 4 tools' existing blue/purple/violet per-tool accents), `big-picture`→emerald (echoes "Community," growth/long-term view). This **superseded the single-color blue/sky Tools identity from the previous amendment** for the four *section* headers/bodies specifically — the page's overall canvas (`THEME.tools.page`, still `bg-blue-200`) and the Dashboard-matching header (`VibrantHeader`, still blue→sky) are unchanged, so the blue/sky "Tools" identity still holds at the page-chrome level, exactly like ROSC's own fuchsia page wash coexists with its 4 differently-colored pillars inside one card. Replaced the previous round's Right-Now-only dark `GlassCard` hero with a new page-local `LightGlass` component (not added to `GlassCard.tsx`, since these are per-section accent colors rather than a module-level design token) — the same gradient-border/blur/ambient-glow recipe rebuilt with a light translucent base (`rgba(255,255,255,0.78)`) instead of near-black, applied uniformly to all four sections' expanded bodies via their own `gradA`/`gradB`. The legacy simple-card branch (Urge Surfer, Resentment Burner) reverted from its prior dark styling back to light, since Right Now's body is no longer dark. Finally, the accordion's default-expanded state (`right-now: true` since the original PROJ-71 ship) changed to all four sections collapsed, per explicit request — David's crisis tools are one tap away rather than zero, a deliberate trade against the persona's "≤2 taps" guidance that the product owner made directly, not a regression to flag.
* **Section headers reverted to a fixed blue, body colors retained (2026-07-21):** Product owner reviewed the per-section header colors from the previous amendment and asked for the headers specifically to go back to the same blue as the Dashboard's "Tools" tile — the request named headers only, not the expanded content. Added a `HEADER_ACCENT` constant to `ToolsHub.tsx` (`#3B82F6`/`#0284C7`, the same `blue-500`/`sky-600` hex pair as `GlassCard.tsx`'s `tools` token) and pointed the header's gradient-border and icon/count-badge inline styles at it instead of each section's own `meta.gradA`/`gradB` — all four collapsed headers now render identically. `PHASE_META`'s per-phase `gradA`/`gradB` (still driving the expanded `LightGlass` body) and `pillBg`/`pillText`/`hoverText` (the "Best for" pill, still per-section) are untouched, so the rose/orange/violet/emerald variety from the previous amendment survives everywhere except the header chrome.
