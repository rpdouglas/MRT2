...
**Status:** 🟡 Active — Phases 1-3 shipped, Phase 4 (stretch — Recovery Games coverage) component-test portion shipped 2026-07-22; manual Subway Test browser pass still outstanding (blocked on a real browser, unavailable in this sandbox — see Phase 4 notes)
...
* **Manual Subway Test browser pass — still outstanding.** The 18-component gap above is closed at the unit/component level, but nobody has driven the actual offline-cache/reconnect-sync behavior (or Fast Lane's close-tab-and-resume flow through `game_saves`) in a real browser. This needs a live browser or Playwright run, which isn't available in this sandbox (same limitation Phase 1 hit for `vault.spec.ts` — see its notes above). First thing to do from a normal dev environment or CI.

---

## 5. QA & Verification 🧪
* [x] **Run Suite:** `npm run test:once` — 594/594 passing as of the Phase 4 component-test addition (579/579 through Phases 1-3). `npm run test:e2e` — the Functions emulator now starts and `verifyVaultPin` loads with its secret resolved correctly (confirmed via a `--debug` emulator trace), but a full Playwright browser run never completed in the sandbox this work was built in (an unrelated, pre-existing sandbox-only network-proxy issue with loopback HTTP between emulators — see Phase 1's notes above). **Not yet observed passing end-to-end in a real environment or CI** — first thing to confirm from VS Code or the next CI run on this branch.
...
* [ ] **Phase 4 manual Subway Test:** not performed — needs a real browser, unavailable in this sandbox (see Phase 4 notes above).
* [ ] **The actual regression check:** temporarily break `verifyVaultPin` (e.g. return a wrong pepper) and confirm `vault.spec.ts` now fails instead of silently passing via fallback — then revert. Not performed — blocked on the same full-browser-run limitation above. This is the concrete proof this project achieved its objective and should be the first thing done once `test:e2e` can run somewhere without that restriction.
...
* [ ] **CI runtime budget:** not yet confirmed — no observed real CI run of `test:e2e` with the Functions emulator added, for the same reason as the regression check above. Should be quick to confirm (Phase 1's estimate: no material change expected) once a real run happens.
