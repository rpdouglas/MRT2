...
## 9. 2026-09-14 Correction — `isAndroidTWA()` broke on its own success path

Found via the first real Play Store build with Play Billing live (PROJ-105): the *first* purchase in a session correctly used Play Billing, but the *next* visit to `/premium` in the same app session fell through to the Stripe branch this ticket exists to prevent — a Play policy violation risk, not just a UX bug.

**Root cause:** `document.referrer.startsWith('android-app://')` (§3 above) is only ever set by the document load Android's launch *intent* itself produces — Chrome never re-sets it for a later same-origin navigation. `PremiumUpgrade.tsx`'s own flows do full-page redirects (`window.location.assign(...)`), most relevantly `handlePlayPurchase`'s post-purchase `window.location.assign('/dashboard')` in PROJ-105. Android/Chrome keeps reusing the same tab across app foreground/background, so that redirect's own document load overwrote `document.referrer` with the app's own previous URL — silently un-detecting the TWA for the rest of that session. This was flagged as an open risk at ship time (§'s original "manual TWA test not yet performed" note) and only surfaced once a real build existed to hit it.

**Fix:** `isAndroidTWA()` (`src/lib/platform.ts`) now caches the first `true` reading in `sessionStorage` (`mrt_is_android_twa`), so a later same-origin navigation that clears the live referrer still returns `true` for the rest of that tab's session. A plain browser tab that never had the real referrer still reads `false` on every call — nothing is cached for a `false` reading. Also benefits `Welcome.tsx`'s TWA cold-launch branch, which reads the same function. 2 new regression tests added to `src/lib/__tests__/platform.test.ts`; `PremiumUpgrade.test.tsx`/`Welcome.test.tsx` unaffected (they mock `isAndroidTWA` directly). Full suite green, lint clean.

## 10. 2026-09-14 Follow-up — §9's cache still lost the signal for an already-logged-in user

Reported again the same day, after §9 shipped: still landed on Stripe. Root cause was narrower than §9's fix covered — the `sessionStorage` cache only protects a *later* reload if some code already called `isAndroidTWA()` once during the pristine referrer window, and until this fix, only `Welcome.tsx` (pre-login) and `PremiumUpgrade.tsx` itself ever called it. An already-authenticated user launches straight onto `/dashboard` (no call there), sees `PWAUpdateBeacon.tsx`'s update toast (it lives inside `AppShell`, mounted on every authenticated route), taps "Reload Now" — that reload's own document load wipes `document.referrer` before anything had cached it. By the time they later reach `/premium`, there's no live referrer and nothing cached either.

**Fix:** call `isAndroidTWA()` eagerly and unconditionally in `src/main.tsx`, before `ReactDOM.createRoot(...).render(...)` — before React mounts, before `AppShell`/`PWAUpdateBeacon` exist, so it always wins the race against any reload an app-mounted component could trigger. The function is idempotent, so `Welcome.tsx`/`PremiumUpgrade.tsx`'s existing calls stay in place as redundant-but-harmless. Considered and rejected: an inline `<script>` in `index.html` earlier still (no practical benefit — `main.tsx` already beats the only thing it needs to, and an inline script can't share `platform.ts`'s `SESSION_KEY` constant without duplicating the literal string); switching the PWA's `registerType` from `'prompt'` to `'autoUpdate'` to remove the user-triggered reload entirely (rejected — reintroduces an unannounced-reload risk for David/Walt that PROJ-19 deliberately chose `'prompt'` to avoid, to fix an unrelated bug). No test file exists for `main.tsx`; verified via `npm run build` (compiles clean) and manual reasoning about mount order — `platform.test.ts`'s existing coverage of `isAndroidTWA()` itself is unaffected since its contract didn't change, only its earliest caller.
