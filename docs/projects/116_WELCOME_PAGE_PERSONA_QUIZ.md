...
## 9. Amendment: TWA-Aware Landing (2026-09-09)

**Source:** a deep-dive review of `PROJ-07`'s (now live) Play Store access flow, run through this file's own Planning Protocol (§7) again — Strategy B approved.

**Finding:** a Play-Store-installed TWA visitor already made the highest-intent decision (install) before this page can even load — `twa-manifest.json`'s `startUrl` is `"/"`, so this *is* their first screen on a logged-out cold launch. Prior to this amendment, `Welcome.tsx` had exactly zero platform branching outside the billing flow (`isAndroidTWA()` was only ever read by `PremiumUpgrade.tsx`/`playBilling.ts`, PROJ-68/105) — a TWA visitor got the identical full marketing hero → feature strip → quiz → showcase → trust statement → auth-card funnel a cold anonymous web visitor gets, including "or get it on Google Play" CTAs on every persona card pointing at the app they're already running.

**Shipped:**
- **`src/components/welcome/AuthCard.tsx`** (new): the embedded email/password + Google sign-up/sign-in form, extracted verbatim from `Welcome.tsx`'s former inline auth-section JSX. Both the full web page and the new TWA landing render this same component/logic — no forked auth code to drift out of sync.
- **`src/components/welcome/WelcomeTrustBar.tsx`** (new): the sticky ZK-badge + crisis-bypass ("Need help?") bar, extracted the same way. **David's crisis-access floor is not conditional on platform** — this bar renders identically on both variants.
- **`src/components/welcome/WelcomeAndroidApp.tsx`** (new): the TWA-only landing — logo, one tagline, `<WelcomeTrustBar>`, `<AuthCard>`. No hero copy, no feature strip, no quiz, no persona showcase, no outbound Play Store link (redundant — they're already in the app).
- **`src/pages/Welcome.tsx`**: branches on `isAndroidTWA()` (same referrer check PROJ-68/105 already use) immediately after all hooks run, returning `<WelcomeAndroidApp>` instead of the full marketing page. Web/iOS-PWA visitors are completely unaffected — same funnel, same components, same copy as before this amendment.
- **Tests:** `src/pages/__tests__/Welcome.test.tsx` (new) — asserts the TWA branch renders the auth form without any quiz/showcase content, the crisis-bypass button is present on both branches, and the full funnel still renders for non-TWA visitors. `npm run check` clean (lint, docs:check-specs, 816/816 tests, build).
- **ZK/schema impact:** none — same N/A status as the rest of this page (§2).
- **Rollback:** fully `git revert`-able, no external/irreversible step.
