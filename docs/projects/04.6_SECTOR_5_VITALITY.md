...
## 3. QA & Verification 🧪
* [x] **Unit Tests:** Verify `calculateVitalityStats` in `gamification.test.ts` accurately calculates streaks bridging across midnight.
* [x] **The Subway Test:** Start the breath pacer while offline. Save the session. Verify it queues locally and syncs to Firestore upon reconnection.
