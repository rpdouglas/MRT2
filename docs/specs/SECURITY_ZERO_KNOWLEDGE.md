...
1.  **Action:** The user triggers "Reset Vault" in the Profile Security tab.
2.  **Shredding:** The app executes a massive batch-delete of all existing documents in `journals` and `workbook_answers`, and deletes the `encryptionSalt` from the user profile.
3.  **Result:** The old ciphertexts are destroyed, preventing orphaned, unreadable data from bloating the database. The user can now create a brand new Vault.
