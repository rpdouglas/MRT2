...
| `mat_doses/{id}` | ✅ Partial | PROJ-111 (MAT Dose-Tracking, Jordan). One-tap daily dose log — doc ID `${uid}_${date}`, upserted via `setDoc`/merge so re-logging the same day updates in place (idempotent, one log/day for v1). `uid`/`loggedAt`/`date`/`isEncrypted` are plaintext (same `tasks`-precedent reasoning: needed for compliance-rate queries, comparable in sensitivity to a completed task); the optional `encryptedNote` free-text field is AES-GCM. Included in `executePinRotation`/`executeCryptoShredding`. `users/{uid}.matModeEnabled`/`customCounterLabel` are plaintext profile metadata (opt-in gate + UI copy, not recovery content) — `customCounterLabel` must never be interpolated into a push notification body (see `functions/src/index.ts`'s `computeMatReminderAlert`, which uses fixed generic copy only). |
| `image_library/{imageId}` | ❌ No | PROJ-113 (Daily Inspirational Image). Admin-uploaded editorial content (Storage path/URL, caption, attribution, tags, rotation bookkeeping) — not user data, same read-any-authenticated/write-admin-only shape as `crossword_puzzles`/`daily_readings`. Backing Storage bucket (`storage.rules`) mirrors the same admin-write/authenticated-read shape — this is the first use of Firebase Storage in the app. |
| `daily_images/{date}` | ❌ No | PROJ-113. Nightly-assigned "today's image" record (one shared doc per calendar date), written by the `generateDailyImage` scheduled Cloud Function from the `image_library` pool — same shape/rationale as `crossword_puzzles`. `users/{uid}.lastSeenDailyImageDate` is plaintext profile metadata (drives the once-per-day Dashboard popup), same category as `lastSeenBuildHash`. |
...
**Known gap, partially fixed (flag if you touch these files):** `WorkbookDetail.tsx` (`analyzeWorkbookContent`), `WorkbookSession.tsx` (`getGeminiCoaching`), and `AudioRecorder.tsx` (`generateAudioAnalysis`) — three of the nine approved Gemini flows above — have **no tier check** (any free or premium user can call them identically; there's no premium-only enforcement point). `PROJ-106` (2026-09) closed the *uncapped-cost* half of this gap with real server-side rate limits in `generateAIInsights` (`functions/src/index.ts`): a 7-day free-tier cooldown on `workbook_analysis`, a 24h free-tier cooldown on `audio_analysis`, and a 15-second all-tier anti-abuse floor on `workbook_coach`. What remains open is purely a product decision — should these three also be premium-gated like the app's other AI flows — not an active cost emergency. Don't extend these call sites without considering whether that decision has been made yet; don't assume the missing tier check is an oversight rather than a deliberate not-yet-decided state.
