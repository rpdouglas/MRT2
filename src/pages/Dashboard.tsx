import { useAuth } from '../contexts/AuthContext'; // removed React import

// ADD 'default' here
export default function Dashboard() {
  const { user } = useAuth();
  
  const sobrietyDate = new Date('2024-07-10'); 
  const today = new Date();
  
  const diffTime = Math.abs(today.getTime() - sobrietyDate.getTime());
  const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
  
  const isFuture = sobrietyDate > today;
        ...
        <div className="px-4 py-5 sm:p-6">
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div className="bg-teal-50 overflow-hidden rounded-lg border border-teal-100 p-4 relative shadow-sm">
              ...
              <dd className="mt-1 text-3xl font-semibold tracking-tight text-teal-900">{displayDays}</dd>
            </div>
